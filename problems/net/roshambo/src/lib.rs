...
pub fn run(ip: IpAddr, port: u16) {
    ...
    let stream_counter = Arc::new(Mutex::new(0));
    let read_stream_counter = Arc::clone(&stream_counter);
    let game_stream_counter = Arc::clone(&stream_counter);

    let stream_vec = Arc::new(Mutex::new(Vec::new()));
    let read_stream_vec = Arc::clone(&stream_vec);
    let game_stream_vec = Arc::clone(&stream_vec);
