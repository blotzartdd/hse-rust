...
fn it_works() {
    ...
    assert_eq!(
        moved_filenames,
        vec![
            "changed_A_filename.cpp",
            "changed_B_filename.rs",
            "changed_C_filename.bin"
        ]
    );
...
fn test_force_flag() {
    ...
    let mut to_directory_path = std::env::current_dir().unwrap();
    to_directory_path.push(
        "tests/test_data/integration_tests_data/force_flag/to_test_folder_with_existing_files",
    );
    let to_pattern = "#1";
...
fn move_files_back(
    from_directory_path: &PathBuf,
    from_pattern: &str,
    to_directory_path: &PathBuf,
    to_pattern: &str,
) {
    run_mmv_with_arguments(
        from_directory_path,
